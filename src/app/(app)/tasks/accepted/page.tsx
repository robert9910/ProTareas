  ...
  return (
    <main className="mx-auto flex max-w-2xl flex-col gap-8 px-4 py-10">
      <div className="flex flex-col gap-6">
        <h1 className="text-3xl font-extrabold tracking-tight text-ink">Por entregar</h1>
        ...
        {!pending.length && (
          <div className="flex flex-col items-center gap-3 rounded-2xl border-2 border-dashed border-ink/15 bg-surface-alt py-16 text-center">
            <span className="flex size-14 items-center justify-center rounded-full bg-status-pending/10">
              <PackageCheck className="size-7 text-status-pending" />
            </span>
        ...
        <ul className="flex flex-col gap-3">
          {pending.map((task) => (
            <li
              key={task.id}
              className={`flex flex-col gap-2 border-l-4 ${taskBorderClass(task.status)} rounded-xl border border-ink/10 bg-white px-4 py-3 shadow-md shadow-ink/5`}
            >
      ...
      {delivered.length > 0 && (
        <div className="flex flex-col gap-3">
          <h2 className="text-lg font-bold text-ink">Entregadas</h2>
          <ul className="flex flex-col gap-3">
            {delivered.map((task) => (
              <li key={task.id}>
                <Link
                  href={`/tasks/${task.id}`}
                  className={`flex flex-col gap-1 border-l-4 ${taskBorderClass(task.status)} rounded-xl border border-ink/10 bg-white px-4 py-3 shadow-md shadow-ink/5 transition hover:-translate-y-0.5 hover:shadow-lg`}
                >
                  <div className="flex items-center justify-between gap-2">
                    <span className="font-bold text-ink">{task.subject}</span>
                    <TaskStatusBadge status={task.status} />
